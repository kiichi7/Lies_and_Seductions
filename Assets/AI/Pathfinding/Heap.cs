...
public class Heap : IList, ICloneable{
	...
	public int Add(object O) {
		//Debug.Log("Heap.Add: FList before addition of : " + ((AStarNode)O).NavmeshPolygon.name);
		//foreach (AStarNode node in FList){
		//	Debug.Log("Heap.Add: " + node.NavmeshPolygon.name + " - " + node.TotalCost);
		//}
		int Return = -1;
		...
		//Debug.Log("Heap.Add: FList after addition: ");
		//foreach (AStarNode node in FList){
		//	Debug.Log("Heap.Add: " + node.NavmeshPolygon.name + " - " + node.TotalCost);
		//}
		return Return;
	...
	public bool Contains(object O) {
		//Debug.Log("Heap.Contains: FList to search for : " + ((AStarNode)O).NavmeshPolygon.name);
		//foreach (AStarNode node in FList){
		//	Debug.Log("Heap.Contains: " + node.NavmeshPolygon.name + " - " + node.TotalCost);
		//}
		//int binarySearchResult = FList.BinarySearch(O, FComparer);
		//Debug.Log("Heap.Contains: binary search result: " + binarySearchResult);
		//bool result = binarySearchResult >= 0;
		bool result = FList.Contains(O);
		//Debug.Log("Heap.Contains: result: " + result);
		return result;
	}
	...
	public object GetEqual(object O){
		return FList[FList.IndexOf(O)];
	}
	...
	public int IndexOf(object Object, int Start)
	{
		...
		while(Result > Start && FList[Result-1].Equals(Object))
			Result--;
		return Result;*/
		return FList.IndexOf(Object, Start);
	}
