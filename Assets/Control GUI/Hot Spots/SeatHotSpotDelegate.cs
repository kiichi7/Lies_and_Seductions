...
public class SeatHotSpotDelegate : HotSpotDelegate {
	...
	public override ArrayList GetAvailableActions () {
		...
		Seat seat = (Seat)GetComponent("Seat");
		CharacterState pcState = (CharacterState)pc.GetComponent("CharacterState");
		if (pcState.GetCurrentSeat() == seat){
			availableActions.Add(new PCStandUpAction(pc, seat));
		} else {
			availableActions.Add(new PCSitDownAction(pc, seat));
		}
